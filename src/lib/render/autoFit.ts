 ...
 *
 * The one exception to whole pixels is `max` itself. Sizes authored by a
 * Figma import are routinely fractional (22.7px), and flooring one that
 * already fits would shrink text that had no need to shrink — a visible,
 * pointless change to a published template. So the unrounded ceiling is
 * tried first; the search below it stays on whole pixels. */
function largestFittingSize(
  ...
  if (max > Math.floor(max) && max >= min && fitsAtSize(measure, input, text, max)) {
    return { fontSizePx: max, overflows: false };
  }
