 ...
 * replaced-element fallback (300x150) as the natural size, which is NOT the
...
/** Fixed image element from a brand logo asset. A logo is chrome, not member
 * content: it lands static, always "contain" (a cropped logo is a defaced
 * logo), and at the artwork's own ratio rather than a generic square. */
...
// ---------------------------------------------------------------------------
// Style clipboard — Figma's copy/paste properties.
// ---------------------------------------------------------------------------
 ...
 *
 * `colorKey` is part of style here, where upstream has no such property: it
 * unlinked field-level brand colour bindings and Signature keeps them, so a
 * copied style has to carry the binding or paste would silently downgrade a
 * palette-bound field to a literal hex. `fontStyle` / `fontStretch` are
 * absent for the opposite reason — they arrive with the font system. */
...
/** facility_logo is image-style: it paints artwork, and none of the text
 * properties mean anything on it. */
const propsForType = (type: FieldType): readonly StyleProp[] =>
  type === "image" || type === "facility_logo"
    ? IMAGE_STYLE_PROPS
    : type === "shape"
      ? SHAPE_STYLE_PROPS
      : TEXT_STYLE_PROPS;
