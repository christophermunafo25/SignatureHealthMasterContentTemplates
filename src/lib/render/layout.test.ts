...
// facility_logo has no upstream equivalent, so nothing in the ported suite
// covers it. The layout pass must treat it as artwork, never as text: it is
// auto-resolved chrome, and a text path would try to measure and shrink it.

describe("facility_logo is never treated as text", () => {
  const canvas = { canvasWidth: 1440, canvasHeight: 1440 };

  it("keeps its authored box exactly, with no hug and no fit", () => {
    const logo = mkField({
      type: "facility_logo",
    ...
    const r = computeLayout({ fields: [logo], ...canvas }, {}, null, measure);
    expect(r.fieldRects.get(logo.id)).toEqual({ x: 120, y: 240, width: 320, height: 160 });
    // No font size is computed for it at all — it is not textual.
    expect(r.fontSizes.has(logo.id)).toBe(false);
  });

  it("keeps its box even under textSizing, which does not apply to artwork", () => {
    const logo = mkField({
      type: "facility_logo",
      fieldKey: "facility_logo",
      width: 320,
    ...
    const r = computeLayout({ fields: [logo], ...canvas }, {}, null, measure);
    expect(r.fieldRects.get(logo.id)!.height).toBe(160);
  });

  it("contributes its full fixed height inside a stack", () => {
    const logo = mkField({
      type: "facility_logo",
      fieldKey: "facility_logo",
      width: 300,
    ...
    const below = mkField({ fieldKey: "caption", width: 300, height: 60, fontSizePx: 20 });
    const group = mkGroup({
      id: "g_logo",
      mode: "stack",
      direction: "vertical",
      ...
      y: 100,
      crossSize: 300,
      children: ["facility_logo", "caption"],
    });
    const r = computeLayout(
      { fields: [logo, below], layoutGroups: [group], ...canvas },
      { caption: "Hello" },
      null,
      measure,
    );
    expect(r.fieldRects.get(logo.id)!.height).toBe(150);
    // The next child clears the logo by exactly the gap.
    expect(r.fieldRects.get(below.id)!.y).toBe(100 + 150 + 20);
  });
});
